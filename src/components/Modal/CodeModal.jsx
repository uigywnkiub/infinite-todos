...
const Modal = ({
                ...
                setErrorAttempts((prev) =>
                    prev.errorCount < 4
                        ? {
                              ...
                              textCount: [...prev.textCount, prev.textCount[prev.errorCount - 1]],
                          }
                        : prev
                );
    ...
    return (
        <>
            {isOpenModal && (
                <div className={styles.modalOverlay} onClick={backgroundClickOverlayCloseHandler}>
                    <div tabIndex={0} ref={modalRef} className={styles.modalWindow}>
                        {isRegister && !isTempLoggedIn && !isBlocked && (
                            <>
                                <h2>Login with code</h2>

                                {!isCodesEqual && !!errorAttempts.errorCount && errorAttempts.errorCount <= 3 && (
                                    <p className={styles.verifyAttemptsErrorText}>
                                        Access to this feature will be restricted for you if you make{" "}
                                ...
                                {!errorAttempts.errorCount && <p>After success you have 5 seconds to locking</p>}
                            </>
                        )}

                        {isBlocked && <NotifPopup text="You've been blocked!" blocked />}

                        {isRegister && isTempLoggedIn && isMatchedCode && <NotifPopup text="Logged!" />}

                        {!isCodeLocalExists && !isBlocked && <h2>Create a code to use it</h2>}

                        {isCodeLocalExists && !isRegister && !isBlocked && <NotifPopup text="Success!" success />}
                        ...
                        {!isRegister && !isBlocked && (
                            <>
                                <div className={styles.codeInput}>
                                    {code.map((digit, idx) => (
                                        <input
                                            ...
                                            className={isCodeLocalExists ? styles.codeInputSuccess : ""}
                                            value={digit}
                                ...
                                {!!countdown && (
                                    <p className={styles.redirectText}>{`Redirect to login in ${countdown} ${
